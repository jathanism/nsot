...
def test_networks_creation_reparenting(site):
    net_8 = models.Network.objects.create(site=site, cidr="10.0.0.0/8")
    net_24 = models.Network.objects.create(site=site, cidr="10.0.0.0/24")
    net_16 = models.Network.objects.create(site=site, cidr="10.0.0.0/16")
    net_0 = models.Network.objects.create(site=site, cidr="0.0.0.0/0")
...
def test_seqential_creation(site):
    net_8 = models.Network.objects.create(site=site, cidr="10.0.0.0/8")
    net_22_1 = models.Network.objects.create(site=site, cidr="10.0.0.0/22")
    net_22_2 = models.Network.objects.create(site=site, cidr="10.0.4.0/22")
    net_22_3 = models.Network.objects.create(site=site, cidr="10.0.8.0/22")
...
def test_network_create_hostbits_set(site):
    with pytest.raises(exc.ValidationError):
        models.Network.objects.create(site=site, cidr="10.0.0.0/0")


def test_network_attributes(site):
    models.Attribute.objects.create(
        site=site, resource_name="Network", name="vlan"
    )

    network = models.Network.objects.create(
        site=site, cidr="10.0.0.0/8", attributes={"vlan": "34"}
    )
...
def test_ip_address_no_network(site):
    with pytest.raises(exc.ValidationError):
        models.Network.objects.create(site=site, cidr="10.0.0.1/32")
...
def test_retrieve_networks(site):
    ...
    net_8 = models.Network.objects.create(
        site=site, cidr="10.0.0.0/8", attributes={"test": "foo"}
    )
    net_24 = models.Network.objects.create(
        site=site, cidr="10.0.0.0/24", attributes={"test": "bar"}
    )
    net_25 = models.Network.objects.create(
        site=site, cidr="10.0.0.0/25", attributes={"test": "baz"}
    )
    ip = models.Network.objects.create(site=site, cidr="10.0.0.1/32")
    ...
    # Get by address
    assert site.networks.get_by_address("10.0.0.0/8") == net_8

    #
    # .get_closest_parent()
    #
    # Closest parent for non-existent 10.0.0.128/32 network should be /24
    assert site.networks.get_closest_parent("10.0.0.128/32") == net_24

    # Closest parent for non-existent 10.0.0.2/32 network should be /25
    assert site.networks.get_closest_parent("10.0.0.2/32") == net_25

    # Matching ip with shorter prefix_length should not match
    with pytest.raises(models.Network.DoesNotExist):
        site.networks.get_closest_parent("10.0.0.2/32", prefix_length=27)

    # Non-existent closest parent should error
    with pytest.raises(models.Network.DoesNotExist):
        site.networks.get_closest_parent("1.0.0.2/32")

    # Invalid prefix_length
    with pytest.raises(exc.ValidationError):
        site.networks.get_closest_parent("10.0.0.2/32", prefix_length="shoe")

    # Invalid CIDR
    with pytest.raises(exc.ValidationError):
        site.networks.get_closest_parent("1")


def test_mptt_methods(site):
    """Test ancestor/children/descendants/root model methods."""
    net_8 = models.Network.objects.create(site=site, cidr="10.0.0.0/8")
    net_12 = models.Network.objects.create(site=site, cidr="10.16.0.0/12")
    net_14 = models.Network.objects.create(site=site, cidr="10.16.0.0/14")
    net_25 = models.Network.objects.create(site=site, cidr="10.16.2.0/25")
    ip1 = models.Network.objects.create(site=site, cidr="10.16.2.1/32")
    ip2 = models.Network.objects.create(site=site, cidr="10.16.2.2/32")
    ...
    assert list(net_25.get_ancestors(ascending=True)) == [
        net_14,
        net_12,
        net_8,
    ]
    ...
    net_192_1 = models.Network.objects.create(site=site, cidr="192.168.1.0/24")
    net_192_2 = models.Network.objects.create(site=site, cidr="192.168.2.0/24")
    ...
    assert list(net_192_2.get_siblings(include_self=True)) == [
        net_8,
        net_192_1,
        net_192_2,
    ]
...
def test_get_next_methods(site):
    ...
    net_25 = models.Network.objects.create(site=site, cidr="10.16.2.0/25")
    net_29 = models.Network.objects.create(site=site, cidr="10.16.2.8/29")
    ip1 = models.Network.objects.create(site=site, cidr="10.16.2.1/32")
    ip2 = models.Network.objects.create(site=site, cidr="10.16.2.2/32")
    ip2 = models.Network.objects.create(site=site, cidr="10.16.2.4/32")
    ip3 = models.Network.objects.create(site=site, cidr="10.16.2.17/32")
    ...
    # A single /28
    assert net_25.get_next_network(28) == [
        ipaddress.ip_network("10.16.2.32/28")
    ]

    # 3x remaining /27 in the /25
    slash27 = ["10.16.2.32/27", "10.16.2.64/27", "10.16.2.96/27"]
    ...
    # A single /32
    assert net_25.get_next_address() == [ipaddress.ip_network("10.16.2.3/32")]

    # 3x /32
    slash32 = ["10.16.2.3/32", "10.16.2.5/32", "10.16.2.6/32"]
    ...
    bad = ("10.16.2.8/32", "10.16.2.15/32")
...
def test_get_next_address_interconnect(site):
    ...
    net_24 = models.Network.objects.create(site=site, cidr="10.20.30.0/24")
    net_31 = models.Network.objects.create(site=site, cidr="10.20.30.0/31")
    net_64 = models.Network.objects.create(site=site, cidr="2001:db8::/64")
    net_127 = models.Network.objects.create(site=site, cidr="2001:db8::/127")
...
def test_reservation(site):
    ...
    reserved = models.Network.objects.create(
        site=site, cidr="192.168.3.0/24", state="reserved"
    )
    ...
    addresses = ["192.168.3.1/32", "192.168.3.2/32", "192.168.3.3/32"]
    assert reserved.get_next_address(num=3, as_objects=False) == addresses


def test_strict_allocation_1(site):
    parent = models.Network.objects.create(site=site, cidr="10.1.2.0/24")
    child = models.Network.objects.create(site=site, cidr="10.1.2.0/25")
    expected = [ipaddress.ip_network("10.1.2.128/32")]
    assert parent.get_next_network(32, strict=True) == expected


def test_strict_allocation_2(site):
    parent = models.Network.objects.create(site=site, cidr="10.2.1.0/24")
    c1 = models.Network.objects.create(site=site, cidr="10.2.1.128/27")
    c2 = models.Network.objects.create(site=site, cidr="10.2.1.64/27")
    c3 = models.Network.objects.create(site=site, cidr="10.2.1.96/27")
    c4 = models.Network.objects.create(site=site, cidr="10.2.1.0/25")
    expected = [
        "10.2.1.160/28",
        "10.2.1.176/28",
        "10.2.1.192/28",
...
def test_strict_allocation_3(site):
    parent = models.Network.objects.create(
        site=site, cidr="2001:db8:abcd:0012::0/96"
    )
    child = models.Network.objects.create(
        site=site, cidr="2001:db8:abcd:0012::0/97"
    )
    expected = [ipaddress.ip_network("2001:db8:abcd:12::8000:0/128")]

...
def test_creation(client, user_client, user, site):
    ...
    assert_error(
        client.create(dev_uri, hostname="invalid hostname"),
    ...
    # Null hostname
    assert_error(
        client.create(dev_uri, hostname=None), status.HTTP_400_BAD_REQUEST
    )
...
def test_bulk_operations(site, client):
    ...
    collection_response = client.post(dev_uri, data=json.dumps(collection))
...
def test_filters(site, client):
    ...
    assert_success(client.retrieve(dev_uri, hostname="foo-bar3"), expected)

    # Test lookup by attributes
    wanted = ["foo-bar2", "foo-bar3"]
    expected = filter_devices(devices, wanted)
    assert_success(client.retrieve(dev_uri, attributes="foo=baz"), expected)
...
def test_set_queries(client, site):
    ...
    expected = filter_devices(devices, wanted)
    assert_success(client.retrieve(query_uri, query="-owner=gary"), expected)
...
def test_update(client, user_client, user, site):
    ...
    params = {"hostname": "foo"}
    assert_error(
        client.update(dev_obj_uri, **params), status.HTTP_400_BAD_REQUEST
    )
    ...
    assert_success(client.update(dev_obj_uri, **params), device)
...
def test_update_natural_key(client, user_client, user, site):
    ...
    assert_success(client.update(dev_natural_uri, **params), device)
    ...
    assert_success(client.update(new_natural_uri, **params), device)
...
def test_detail_routes(site, client):
    ...
    natural_ifaces_uri = reverse(
        "device-interfaces", args=(site.id, dev1["hostname"])
    )

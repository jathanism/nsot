...
class TestProtocolType:
    ...
    def test_name_uniqueness(self, site, client, bgp_type):
        ...
        assert_error(
            client.create(pt_uri, name="bgp"), status.HTTP_400_BAD_REQUEST
        )
    ...
    def test_required_attributes_site(self, site, client, bgp_type):
        ...
        assert_error(
            client.update(pt_obj_uri, **pt), status.HTTP_400_BAD_REQUEST
        )

    def test_required_attributes_resource(
        self, site, client, bgp_type, asn_attribute
    ):
    ...
    def test_update_required_attributes(
        self, site, client, bgp_type, asn_attribute
    ):
        ...
        assert_error(
            client.update(pt_obj_uri, **payload), status.HTTP_400_BAD_REQUEST
        )
    ...
    def test_partial_update(self, site, client, bgp_type):
        ...
        assert_success(
            client.partial_update(obj_uri, name="patched"), bgp_type
        )
    ...
    def test_filtering(self, site, client, bgp_type):
        ...
        assert_success(
            client.retrieve(pt_uri, description="bgp2"), [bgp2_type]
        )
...
class ProtocolTestCase:
    ...
    def isis_protocol(self, site, client, device, isis_type, interface):
        ...
        proto_resp = client.create(
            ...
            device=device["hostname"],
            type=isis_type["name"],
            attributes={"metric": "100"},
            interface=interface["name_slug"],
            description="IS-IS",
        )
        proto = get_result(proto_resp)
        return proto
...
class TestRetrieval(ProtocolTestCase):
    ...
    def test_filters_by_id(
        ...
        assert_success(
            client.retrieve(proto_uri, description="IS-IS"), expected
        )

        # Filter by type
        expected = [bgp_protocol]
        assert_success(
            client.retrieve(proto_uri, type=bgp_type["id"]), expected
        )

        # Filter by device
        expected = [bgp_protocol, isis_protocol]
        assert_success(
            client.retrieve(proto_uri, device=device["id"]), expected
        )

        # Filter by circuit
        expected = [bgp_protocol]
        assert_success(
            client.retrieve(proto_uri, circuit=circuit["id"]), expected
        )

        # Filter by interface
        expected = [isis_protocol]
        assert_success(
            client.retrieve(proto_uri, interface=interface["id"]), expected
        )

    def test_filters_by_natural_key(
        self,
        site,
        client,
        device,
        ...
        expected = [bgp_protocol, isis_protocol]
        assert_success(
            client.retrieve(proto_uri, device=device["hostname"]), expected
        )

        # Filter by circuit
        expected = [bgp_protocol]
        assert_success(
            client.retrieve(proto_uri, circuit=circuit["name_slug"]), expected
        )
    ...
    def test_set_queries(self, site, client, bgp_protocol, isis_protocol):
        ...
        expected = [bgp, isis]
        assert_success(
            client.retrieve(query_uri, query="peer_as=19679"), expected
        )
        ...
        expected = [isis]
        assert_success(
            client.retrieve(query_uri, query="-asn=12345"), expected
        )

        # UNION: asn=12345 +metric=100
        expected = [bgp, isis]
        assert_success(
            client.retrieve(query_uri, query="asn=12345 +metric=100"), expected
        )
...
class TestUpdate(ProtocolTestCase):
    ...
    def test_update_related_natural_key(
        self,
        site,
        client,
        bgp_protocol,

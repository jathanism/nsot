...
def test_no_user(live_server, site):
    ...
    url = f"{live_server.url}{site_uri}"
...
def test_invalid_user(live_server):
    ...
    assert_error(
        requests.get(url, headers=headers), status.HTTP_400_BAD_REQUEST
    )
...
def test_get_auth_token_valid(live_server, user):
    ...
    url = f"{live_server.url}{auth_uri}"
...
def test_verify_auth_token_invalid(live_server, user):
    ...
    assert_error(
        requests.post(verify_url, headers=headers), status.HTTP_400_BAD_REQUEST
    )
...
def test_verify_auth_token_valid(live_server, site, user):
    ...
    headers.update({"Authorization": f"AuthToken {user.email}:{auth_token}"})
...
def test_valid_auth_token(live_server, user):
    ...
    assert_success(requests.get(site_url, headers=headers), expected)

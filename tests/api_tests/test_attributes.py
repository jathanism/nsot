...
def test_creation(client, site):
    ...
    assert_error(
        client.create(attr_uri, resource_name="Network", name="invalid attr1"),
...
def test_bulk_operations(client, site):
    ...
    collection_response = client.post(attr_uri, data=json.dumps(collection))
    ...
    assert_success(client.get(attr_uri), payload)
...
def test_update(client, site):
    ...
    assert_success(client.update(attr_obj_uri, **params), attr1)
    ...
    assert_success(client.update(attr_obj_uri, **params), attr2)

    # Reset the object back to it's initial state!
    assert_success(client.update(attr_obj_uri, **attr), attr)
...
def test_partial_update(site, client):
    ...
    assert_success(client.partial_update(attr_pk_uri, **params), payload)

    # Update constraints
    params = {
        "constraints": {"allow_empty": True, "pattern": "", "valid_values": []}
    }
    payload.update(params)
    assert_success(client.partial_update(attr_pk_uri, **params), payload)
...
def test_deletion(client, site):
    ...
    assert_error(client.delete(attr_obj_uri), status.HTTP_409_CONFLICT)

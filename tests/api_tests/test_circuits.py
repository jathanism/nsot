...
def test_creation(site, client):
    ...
    assert_error(
        client.create(cir_uri, endpoint_a=if_a["id"], endpoint_z=if_z["id"]),
        status.HTTP_400_BAD_REQUEST,
    )

    # Try to create another circuit with interfaces A/Z inverted.
    assert_error(
        client.create(cir_uri, endpoint_a=if_z["id"], endpoint_z=if_a["id"]),
        status.HTTP_400_BAD_REQUEST,
    )
...
def test_creation_toplevel(site, client):
    ...
    # Interfaces
    if_a = get_result(client.create(ifc_uri, device=dev_a["id"], name="eth0"))
    if_z = get_result(client.create(ifc_uri, device=dev_z["id"], name="eth0"))
    ...
    if_a2 = get_result(client.create(ifc_uri, device=dev_a["id"], name="eth1"))
    if_z2 = get_result(client.create(ifc_uri, device=dev_z["id"], name="eth1"))
    assert_error(
        client.create(cir_uri, endpoint_a=if_a2["id"], endpoint_z=if_z2["id"]),
        status.HTTP_400_BAD_REQUEST,
    )
...
def test_bulk_operations(site, client):
    ...
    collection_response = client.post(cir_uri, data=json.dumps(collection))
...
def test_update(site, client):
    ...
    assert_success(client.update(cir_obj_uri, **params), payload)
    ...
    assert_error(
        client.update(cir_obj_uri, **params), status.HTTP_400_BAD_REQUEST
    )
    ...
    assert_success(client.update(cir_natural_uri, **params), payload)
    ...
    assert_error(
        client.update(cir2_obj_uri, **params), status.HTTP_400_BAD_REQUEST
    )
...
def test_filters(site, client):
    ...
    assert_success(
        client.retrieve(cir_uri, name="foo-bar1:eth0_foo-bar2:eth0"), expected
    )
    ...
    expected = filter_circuits(circuits, wanted)
    # by ID
    assert_success(client.retrieve(cir_uri, endpoint_a=if_a2["id"]), expected)
    # by natural key
    assert_success(
        client.retrieve(cir_uri, endpoint_a=if_a2["name_slug"]), expected
    )
    ...
    expected = filter_circuits(circuits, wanted)
    # by ID
    assert_success(client.retrieve(cir_uri, endpoint_z=if_z1["id"]), expected)
    # by natural key
    assert_success(
        client.retrieve(cir_uri, endpoint_z=if_z1["name_slug"]), expected
    )
    ...
    expected = filter_circuits(circuits, wanted)
    assert_success(
        client.retrieve(cir_uri, attributes=["vendor=acme"]), expected
    )
...
def test_set_queries(client, site):
    ...
    wanted = [cir1, cir2]
    expected = filter_circuits(circuits, wanted)
    assert_success(client.retrieve(query_uri, query="vendor=acme"), expected)
    # INTERSECTION: cid=abc246 vendor=acme
    wanted = [cir2]
    expected = filter_circuits(circuits, wanted)
    assert_success(
        client.retrieve(query_uri, query="cid=abc246 vendor=acme"), expected
    )
    ...
    assert_success(
        client.retrieve(
            query_uri, query="cid=abc123 vendor=acme", unique=True
        ),
        expected,
    )

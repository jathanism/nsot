...
def test_creation(site, client):
    ...
    assert_error(
        client.create(
            ifc_uri, device=dev1["id"], name="eth0.1", parent_id=ifc1["id"]
        ),
        status.HTTP_400_BAD_REQUEST,
    )
...
def test_bulk_operations(site, client):
    ...
    collection_response = client.post(ifc_uri, data=json.dumps(collection))
...
def test_update(site, client):
    ...
    assert_error(
        client.update(ifc2_obj_uri, **params), status.HTTP_400_BAD_REQUEST
    )

    # (device_id, name) must be unique
    params["name"] = "eth0"
    assert_error(
        client.update(ifc2_obj_uri, **params), status.HTTP_409_CONFLICT
    )
...
def test_partial_update(site, client):
    ...
    assert_error(
        client.update(ifc_pk_uri, **params), status.HTTP_400_BAD_REQUEST
    )
...
def test_filters(site, client):
    ...
    assert_success(client.retrieve(ifc_uri, name="eth0"), expected)

    # Test filter by device
    wanted = [dev1_eth0, dev1_eth1]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(client.retrieve(ifc_uri, device=dev1["id"]), expected)

    # Test filter by device__hostname (on device)
    wanted = [dev1_eth0, dev1_eth1]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(
        client.retrieve(ifc_uri, device__hostname=dev1["hostname"]), expected
    )
    ...
    expected = filter_interfaces(interfaces, wanted)
    assert_success(
        client.retrieve(ifc_uri, device_hostname=dev1["hostname"]), expected
    )

    # Test filter by speed
    wanted = [dev1_eth1]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(client.retrieve(ifc_uri, speed=40000), expected)

    # Test filter by type
    wanted = [dev1_eth1, dev2_eth1]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(client.retrieve(ifc_uri, type=161), expected)

    # Test filter by description
    wanted = [dev2_eth0]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(
        client.retrieve(ifc_uri, description="foo-bar2:eth0"), expected
    )
    ...
    expected = filter_interfaces(interfaces, wanted)
    assert_success(
        client.retrieve(ifc_uri, parent_id=dev2_eth0["id"]), expected
    )

    # Test filter by attributes
    wanted = [dev1_eth0]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(client.retrieve(ifc_uri, attributes=["vlan=100"]), expected)
...
def test_set_queries(client, site):
    ...
    expected = filter_interfaces(interfaces, wanted)
    assert_success(
        client.retrieve(query_uri, query="vlan=300 scope=region"), expected
    )

    # DIFFERENCE: -scope=region
    wanted = [dev1_eth1, dev2_eth1]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(client.retrieve(query_uri, query="-scope=region"), expected)

    # UNION: scope=global +vlan=400
    wanted = [dev1_eth1, dev2_eth1]
    expected = filter_interfaces(interfaces, wanted)
    assert_success(
        client.retrieve(query_uri, query="scope=global +vlan=400"), expected
    )
    ...
    assert_error(
        client.retrieve(
            query_uri, query="scope=global +vlan=400", unique=True
        ),
        status.HTTP_400_BAD_REQUEST,
    )

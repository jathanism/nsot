...
def test_creation(live_server, user, site):
    ...
    # Invalid permissions
    assert_error(
        user_client.create(net_uri, cidr=cidr, attributes={"attr1": "foo"}),
    ...
    # Null cidr
    assert_error(
        admin_client.create(net_uri, cidr=None), status.HTTP_400_BAD_REQUEST
    )
...
def test_bulk_operations(site, client):
    ...
    collection_response = client.post(net_uri, data=json.dumps(collection))
...
def test_filters(site, client):
    ...
    assert_success(client.retrieve(net_uri, cidr="10.0.0.0/8"), expected)

    # Test lookup by attributes
    wanted = ["192.168.0.0/16", "172.16.0.0/12"]
    expected = filter_networks(networks, wanted)
    assert_success(client.retrieve(net_uri, attributes="foo=baz"), expected)
    ...
    expected = filter_networks(networks, wanted)
    assert_success(
        client.retrieve(net_uri, network_address="169.254.0.0"), expected
    )

    # Test lookup by prefix_length
    wanted = ["192.168.0.0/16", "169.254.0.0/16"]
    expected = filter_networks(networks, wanted)
    assert_success(client.retrieve(net_uri, prefix_length=16), expected)
    ...
    expected = [ipv6]
    assert_success(client.retrieve(net_uri, ip_version="6"), expected)
...
def test_set_queries(client, site):
    ...
    expected = filter_networks(networks, wanted)
    assert_success(client.retrieve(query_uri, query="foo=bar"), expected)

    # INTERSECTION: foo=bar owner=jathan
    wanted = ["169.254.0.0/16"]
    expected = filter_networks(networks, wanted)
    assert_success(
        client.retrieve(query_uri, query="foo=bar owner=jathan"), expected
    )
    ...
    expected = filter_networks(networks, wanted)
    assert_success(
        client.retrieve(query_uri, query="cluster +foo=baz"), expected
    )

    # Single IP result.
    wanted = ["192.168.0.1/32"]
    expected = filter_networks(networks, wanted)
    assert_success(client.retrieve(query_uri, query="vlan=300"), expected)
    ...
    assert_success(
        client.retrieve(
            query_uri, query="hostname=foo-bar1 vlan=300", unique=True
        ),
        expected,
    )
...
def test_update(live_server, user, site):
    ...
    admin_client.create(attr_uri, resource_name="Network", name="attr1")
    net_resp = admin_client.create(
        net_uri, cidr=cidr, attributes={"attr1": "foo"}, state="reserved"
    )
    ...
    assert_success(admin_client.update(net_obj_uri, **params), net)
    ...
    net_natural_uri = site.detail_uri("network", id=cidr)
    assert_success(admin_client.update(net_natural_uri, **params), net)
...
def test_partial_update(site, client):
    ...
    net_resp = client.create(net_uri, cidr=cidr, attributes={"attr1": "foo"})
    ...
    assert_success(client.partial_update(net_pk_uri, **params), net)

    # And just to make sure a PUT with the same payload fails...
    assert_error(
        client.update(net_pk_uri, **params), status.HTTP_400_BAD_REQUEST
    )
...
def test_force_deletion(site, client):
    ...
    assert_error(
        client.retrieve(slash23_parent_uri), status.HTTP_404_NOT_FOUND
    )

    # Force delete /23 will fail, because it has no parent and children are leaf nodes.
    assert_error(
        client.destroy(net1_obj_uri, force_delete=True),
...
def test_get_next_detail_routes(site, client):
    ...
    networks = ["10.16.2.32/27", "10.16.2.64/27", "10.16.2.96/27"]
    assert_success(client.retrieve(uri, prefix_length=27, num=3), networks)
    assert_success(
        client.retrieve(natural_uri, prefix_length=27, num=3), networks
    )
    ...
    assert_error(
        client.retrieve(uri, prefix_length="ralph"),
        status.HTTP_400_BAD_REQUEST,
    )
    assert_error(
        client.retrieve(uri, prefix_length=14), status.HTTP_400_BAD_REQUEST
    )
    assert_error(
        client.retrieve(uri, prefix_length=65), status.HTTP_400_BAD_REQUEST
    )
    ...
    assert_error(
        client.retrieve(natural_uri, prefix_length=65),
        status.HTTP_400_BAD_REQUEST,
    )
    ...
    # A single /32
    assert_success(client.retrieve(uri), ["10.16.2.3/32"])
    assert_success(client.retrieve(natural_uri), ["10.16.2.3/32"])

    # 3x /32
    addresses = ["10.16.2.3/32", "10.16.2.5/32", "10.16.2.6/32"]
    ...
    assert_error(
        client.retrieve(uri, num="potato"), status.HTTP_400_BAD_REQUEST
    )
    assert_error(
        client.retrieve(natural_uri, num="potato"), status.HTTP_400_BAD_REQUEST
    )
...
def test_next_network_allocation(site, client):
    ...
    client.post(uri, params={"prefix_length": "32"})
    assert_success(client.retrieve(uri, prefix_length=32), ["10.1.2.2/32"])

    client.post(uri, params={"prefix_length": "32", "reserve": "True"})

    uri = reverse("network-reserved", args=(site.id,))
    assert get_result(client.retrieve(uri))[0]["network_address"] == "10.1.2.2"
...
def test_next_address_allocation(site, client):
    ...
    client.post(uri, params={"reserve": "True"})
...
def test_closest_parent_detail_route(site, client):
    ...
    bad_closest_uri = reverse("network-closest-parent", args=(site.id, 1))

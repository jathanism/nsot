...
def test_network_bug_issues_34(client, site):
    ...
    assert_success(
        client.retrieve(net_uri, attributes="hostname=foo-bar1"), expected
    )
...
def test_options_bug_issues_126(client, site):
    ...
    expected = ["actions", "description", "name", "parses", "renders"]
...
def test_duplicate_400_issues_142(client, site):
    ...
    assert_error(
        client.create(net_uri, cidr="10.0.0.0/8"), status.HTTP_400_BAD_REQUEST
    )
...
def test_natural_lookup_without_site(client, site):
    ...
    assert_error(client.get(root_dev_detail_uri), status.HTTP_400_BAD_REQUEST)
    ...
    assert_error(client.get(root_net_detail_uri), status.HTTP_400_BAD_REQUEST)
...
def test_attribute_lookup_list_view_issues_169(client, site):
    ...
    expected = [dev1]
    assert_success(
        client.retrieve(site1_dev_uri, attributes="owner=jathan"), expected
    )
...
def test_interface_assign_address_500_issues_168(client, site):
    ...
    assert_success(client.put(ifc_detail_uri, data=json.dumps(ifc)), expected)
...
def test_bogus_url_raises_404(client, site):
    ...
    assert_error(client.get(bogus_url), status.HTTP_404_NOT_FOUND)
